...
```javascript
  ...
  function inheritObject(sonClass, FatherClass) {
    let clone = Object.create(FatherClass.prototype) //基于父原型创建新对象 这样操作会丢失构造函数默认的constructor属性
    console.log(clone)
...
这种方法只调用了一次父类构造函数，instanceof 和isPrototypeOf()也能正常使用。
